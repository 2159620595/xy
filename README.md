...
docker run -d \
  -p 8080:8080 \
  --restart always \
  -v $PWD/xianyu-auto-reply/:/app/backend/data/ \
...
# 国内用户（阿里云）
docker run -d -p 8080:8080 --restart always -v %cd%/xianyu-auto-reply/:/app/backend/data/ --name xianyu-auto-reply registry.cn-shanghai.aliyuncs.com/zhinian-software/xianyu-auto-reply:latest

# 国际用户（Docker Hub）
docker run -d -p 8080:8080 --restart always -v %cd%/xianyu-auto-reply/:/app/backend/data/ --name xianyu-auto-reply zhinianblog/xianyu-auto-reply:latest
```
...
### 5. Docker容器启动失败
如果遇到 `exec /app/backend/entrypoint.sh: no such file or directory` 错误：
