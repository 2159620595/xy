...
if not exist "backend\\entrypoint.sh" (
    echo %ERROR_PREFIX% backend\entrypoint.sh 文件不存在，Docker容器将无法启动
    echo %INFO_PREFIX% 请确保项目文件完整
    pause
    exit /b 1
) else (
    echo %SUCCESS_PREFIX% backend\entrypoint.sh 文件已存在
)

if not exist "backend\\global_config.yml" (
    echo %ERROR_PREFIX% backend\global_config.yml 配置文件不存在
    echo %INFO_PREFIX% 请确保配置文件存在
    pause
    exit /b 1
) else (
    echo %SUCCESS_PREFIX% backend\global_config.yml 配置文件已存在
)
