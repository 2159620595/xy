...
const JUDIAN_API_BASE = normalizeBase(import.meta.env.VITE_JUDIAN_API_BASE, '/netdisk_api/judian_api')
...
const JUDIAN_SESSION_BOOTSTRAP_URL = buildUrl(NETDISK_API_BASE, '/judian/session')

let sessionSyncPromise: Promise<boolean> | null = null
let trustedToken = ''
let trustedAt = 0

const parseStoredJson = <T = Record<string, unknown>>(key: string): T | null => {
...
const canReuseTrustedToken = (token: string) =>
  Boolean(token && token === trustedToken && Date.now() - trustedAt < SESSION_TRUST_WINDOW_MS)
...
export function setJudianSession(payload: { token?: string; user_id?: number; username?: string; is_admin?: boolean }) {
