...
import { clearJudianSession, ensureJudianSession, getJudianToken, getMainUser } from '@/utils/judian-auth'
...
export const JUDIAN_API_BASE = normalizeBase(import.meta.env.VITE_JUDIAN_API_BASE, '/netdisk_api/judian_api')
export const JUDIAN_API_TIMEOUT_MS = parsePositiveInt(import.meta.env.VITE_JUDIAN_API_TIMEOUT_MS, 30000)
...
judianClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (isUnauthorized && !getMainUser()) {
      clearJudianSession()
      if (typeof window !== 'undefined' && window.location.pathname.startsWith('/judian')) {
...
export const judianApi = {
  ...
  loginAccount: (data: Record<string, unknown>) =>
    judianClient.post('/accounts/login', data, { timeout: JUDIAN_LOGIN_API_TIMEOUT_MS }),
  reloginAccount: (rowId: number | string) =>
    judianClient.post(`/accounts/${rowId}/relogin`, null, { timeout: JUDIAN_LOGIN_API_TIMEOUT_MS }),
  updateAccount: (rowId: number | string, data: Record<string, unknown>) => judianClient.put(`/accounts/${rowId}`, data),
  deleteAccount: (rowId: number | string) => judianClient.delete(`/accounts/${rowId}`),
  ...
  createCard: (data: Record<string, unknown>) => judianClient.post('/cards', data),
  updateCard: (cardId: number | string, data: Record<string, unknown>) => judianClient.put(`/cards/${cardId}`, data),
  deleteCard: (cardId: number | string) => judianClient.delete(`/cards/${cardId}`),
  importCdKeys: (data: Record<string, unknown>) => judianClient.post('/cdkeys/import', data),
  importCdKeysFromNetdisk: (data: Record<string, unknown>) => judianClient.post('/cdkeys/import/netdisk', data),
  importCardCodesFromNetdisk: (cardId: number | string, data: Record<string, unknown>) =>
    judianClient.post(`/cards/${cardId}/import/netdisk`, data),
  importCardCodesFromJudian: (cardId: number | string, data: Record<string, unknown>) =>
    judianClient.post(`/cards/${cardId}/import/judian`, data),
  generateCdKeys: (data: Record<string, unknown>) => judianClient.post('/cdkeys', data),
  updateCdKey: (rowId: number | string, data: Record<string, unknown>) => judianClient.put(`/cdkeys/${rowId}`, data),
  ...
  publicUnlockDetail: (sessionId: string) => axios.get(buildJudianApiUrl(`/public/unlock/${sessionId}`)),
  publicUnlockBatchDetail: (sessionId: string) => axios.get(buildJudianApiUrl(`/public/unlock/${sessionId}/batch`)),
  ...
  publicUnlockBatchPurchase: (sessionId: string, data: Record<string, unknown>) =>
    axios.post(buildJudianApiUrl(`/public/unlock/${sessionId}/batch`), data),
  publicUnlockConfirm: (sessionId: string) => axios.post(buildJudianApiUrl(`/public/unlock/${sessionId}/confirm`)),
  publicUnlockComplete: (sessionId: string) => axios.post(buildJudianApiUrl(`/public/unlock/${sessionId}/complete`)),
}
