...
init_config() {
    ...
    if [ ! -f "backend/entrypoint.sh" ]; then
        print_error "backend/entrypoint.sh 文件不存在，Docker容器将无法启动"
        print_info "请确保项目文件完整"
        exit 1
    else
        print_success "backend/entrypoint.sh 文件已存在"
    fi

    if [ ! -f "backend/global_config.yml" ]; then
        print_error "backend/global_config.yml 配置文件不存在"
        print_info "请确保配置文件存在"
        exit 1
    else
        print_success "backend/global_config.yml 配置文件已存在"
    fi
...
show_access_info() {
    ...
    echo "🔐 登录信息:"
    echo "   用户名: admin"
    echo "   密码:   请查看环境变量 ADMIN_PASSWORD 或 data/initial_admin_password.txt"
    echo ""
    echo "📊 管理命令:"
    echo "   查看状态: $0 status"
    echo "   查看日志: $0 logs"
...
backup_data() {
    ...
    cp backend/global_config.yml "$backup_dir/" 2>/dev/null || true
