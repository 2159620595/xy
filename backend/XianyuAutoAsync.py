...
from app_logging import configure_logging, get_logger, list_log_files
...
def _is_verbose_token_refresh_logging_enabled() -> bool:
    return _as_bool(
        os.getenv("VERBOSE_TOKEN_REFRESH_LOGGING"),
        _as_bool(LOG_CONFIG.get("verbose_token_refresh_logging"), False),
    )


def _is_verbose_websocket_logging_enabled() -> bool:
    return _as_bool(
        os.getenv("VERBOSE_WEBSOCKET_LOGGING"),
        _as_bool(LOG_CONFIG.get("verbose_websocket_logging"), False),
    )


def _is_verbose_startup_retry_logging_enabled() -> bool:
    return _as_bool(
        os.getenv("VERBOSE_STARTUP_RETRY_LOGGING"),
        _as_bool(LOG_CONFIG.get("verbose_startup_retry_logging"), False),
    )


def _truncate_log_text(value: Any, limit: int = 240) -> str:
    text = str(value or "")
    if len(text) <= limit:
        return text
...
def _extract_ret_messages(res_json: Any) -> list[str]:
    if not isinstance(res_json, dict):
        return [str(res_json)] if res_json else []

    ret_value = res_json.get("ret", [])
    if isinstance(ret_value, list):
        return [str(item) for item in ret_value if item]
    if ret_value:
        return [str(ret_value)]
    return []


def _summarize_token_refresh_response(status_code: int | None, res_json: Any) -> str:
    parts: list[str] = []
    if status_code is not None:
        parts.append(f"status={status_code}")

    ret_messages = _extract_ret_messages(res_json)
    if ret_messages:
        parts.append(
            "ret=" + " | ".join(_truncate_log_text(item, 120) for item in ret_messages[:2])
        )
    ...
    if isinstance(res_json, dict):
        ...
        if isinstance(data, dict):
            if data.get("accessToken"):
                parts.append("has_access_token=true")
            if data.get("url"):
                parts.append(f"url={_truncate_log_text(data['url'], 160)}")

    return "; ".join(parts) if parts else _truncate_log_text(res_json, 240)


def _log_startup_retry_traceback(cookie_id: str, context: str) -> None:
    if _is_verbose_startup_retry_logging_enabled():
        logger.debug(f"【{cookie_id}】{context}详细堆栈:\n{traceback.format_exc()}")
        return
...
class AutoReplyPauseManager:
    """自动回复暂停管理器"""
    def __init__(self):
        # 存储每个账号下每个chat_id的暂停信息 {(cookie_id, chat_id): pause_until_timestamp}
        self.paused_chats = {}

    def _build_pause_key(self, chat_id: str, cookie_id: str):
    ...
    def pause_chat(self, chat_id: str, cookie_id: str):
        ...
        pause_key = self._build_pause_key(chat_id, cookie_id)
...
class XianyuLive:
    ...
    def _reset_background_tasks(self):
        ...
        """
        logger.debug(f"【{self.cookie_id}】准备重置后台任务引用（仅重置依赖WebSocket的任务）")
        
        # 只处理心跳任务（依赖WebSocket，需要重启）
        if self.heartbeat_task:
            status = "已完成" if self.heartbeat_task.done() else "运行中"
            logger.debug(f"【{self.cookie_id}】发现心跳任务（状态: {status}），需要重置（因为依赖WebSocket连接）")
            # 尝试取消心跳任务（但不等待）
            ...
            logger.debug(f"【{self.cookie_id}】心跳任务引用已重置")
        else:
            logger.debug(f"【{self.cookie_id}】没有心跳任务需要重置")
        ...
        if other_tasks_status:
            logger.debug(f"【{self.cookie_id}】其他任务继续运行（不依赖WebSocket）: {', '.join(other_tasks_status)}")
        else:
            logger.debug(f"【{self.cookie_id}】没有其他任务在运行")
        
        logger.debug(f"【{self.cookie_id}】任务重置完成，可以立即创建新的心跳任务")
    ...
    async def _cancel_background_tasks(self):
        ...
        try:
            ...
            if not tasks_to_cancel:
                logger.debug(f"【{self.cookie_id}】没有后台任务需要取消（所有任务已完成或不存在）")
                # 立即重置任务引用
                self.heartbeat_task = None
            ...
            logger.debug(f"【{self.cookie_id}】开始取消 {len(tasks_to_cancel)} 个未完成的后台任务")
            
            # 取消所有任务
            for task_name, task in tasks_to_cancel:
                try:
                    if task.done():
                        logger.debug(f"【{self.cookie_id}】任务已完成，跳过取消: {task_name}")
                    else:
                        task.cancel()
            ...
            logger.debug(f"【{self.cookie_id}】等待 {len(tasks)} 个任务响应取消信号")
            ...
            try:
                ...
                for task_name, task in tasks_to_cancel:
                    ...
                    logger.debug(f"【{self.cookie_id}】任务状态: {task_name} - {status}")
                
                if not pending_tasks_list:
                    logger.debug(f"【{self.cookie_id}】所有任务已完成，无需等待")
                else:
                    logger.debug(f"【{self.cookie_id}】等待 {len(pending_tasks_list)} 个未完成任务响应（超时时间: {wait_timeout}秒）")
                    try:
                        # 使用 wait 等待任务完成，设置超时
                        ...
                        logger.debug(f"【{self.cookie_id}】asyncio.wait() 返回，耗时 {elapsed:.3f}秒，已完成: {len(done)}，未完成: {len(pending)}")
                        ...
                        for task_name, task in tasks_to_cancel:
                            if task in done:
                                try:
                                    ...
                                    logger.debug(f"【{self.cookie_id}】✅ 任务已成功取消: {task_name}")
                                except Exception as e:
                        ...
                        if pending:
                                ...
                                if task in pending:
                                    ...
                                    if task.done():
                                        try:
                                            ...
                                            logger.debug(f"【{self.cookie_id}】任务在等待期间被取消: {task_name}")
                                        except Exception as e:
                                    ...
                                    for task_name, task in tasks_to_cancel:
                                        if task in done2:
                                            try:
                                                ...
                                                logger.debug(f"【{self.cookie_id}】任务在二次等待期间被取消: {task_name}")
                                            except Exception as e:
                            ...
                            logger.debug(f"【{self.cookie_id}】所有后台任务已取消（耗时 {elapsed:.3f} 秒）")
            ...
            logger.debug(f"【{self.cookie_id}】任务取消流程完成，继续重连流程")
            ...
            self.cleanup_task = None
            self.cookie_refresh_task = None
            logger.debug(f"【{self.cookie_id}】后台任务引用已全部重置")
    ...
    async def _cleanup_playwright_cache(self):
        ...
        try:
            ...
            if total_cleaned > 0:
                logger.debug(f"【{self.cookie_id}】Playwright缓存清理完成: 删除了 {total_cleaned} 个文件/目录，释放 {total_size_mb:.2f} MB")
            else:
    ...
    async def _cleanup_old_logs(self, retention_days: int = 7):
        ...
        try:
            ...
            for log_file in list_log_files(include_archives=True):
                try:
    ...
    def __init__(self, cookies_str=None, cookie_id: str = "default", user_id: int = None):
        """初始化闲鱼直播类"""
        logger.debug(f"【{cookie_id}】开始初始化XianyuLive")
        ...
        logger.debug(f"【{cookie_id}】解析cookies")
        self.cookies = trans_cookies(cookies_str)
        logger.debug(f"【{cookie_id}】cookies解析完成，字段数: {len(self.cookies)}")
        ...
        logger.debug(f"【{cookie_id}】用户ID: {self.myid}")
        ...
        try:
            ...
            if content_json_str:
                try:
                    ...
                    logger.debug(f"【{self.cookie_id}】解析内容JSON失败: {parse_e}")
            ...
            if not order_id and content_json_str:
                try:
                    ...
                    logger.debug(f"【{self.cookie_id}】解析dynamicOperation JSON失败: {parse_e}")
            ...
            if not order_id:
                try:
                    ...
                    logger.debug(f"【{self.cookie_id}】在消息字符串中搜索订单ID失败: {search_e}")
                ...
                try:
                    ...
                    for i in range(quantity_to_send):
                        try:
                            ...
                            delivery_payload = await self._auto_delivery(item_id, item_title, order_id, send_user_id)
                            if delivery_payload:
                                delivery_payloads.append(delivery_payload)
                                success_count += 1
                    ...
                    if delivery_payloads:
                        ...
                        # 发送所有获取到的发货内容
                        sent_count = 0
                        for i, delivery_payload in enumerate(delivery_payloads):
                            try:
                                delivery_content = delivery_payload['content']
                                # 检查是否是图片发送标记
                        ...
                        if sent_count > 0:
                            try:
                                from db_manager import db_manager
                                db_manager.increment_delivery_times(delivery_payloads[0]['rule_id'])
                                logger.info(f"自动发货记录次数已增加: 规则ID={delivery_payloads[0]['rule_id']}, 本次成功发送={sent_count}")

                                delivery_content_lines = []
                                for payload in delivery_payloads:
                                    payload_content = payload['content']
                                    if payload_content.startswith("__IMAGE_SEND__"):
                                        image_data = payload_content.replace("__IMAGE_SEND__", "")
                                        image_url = image_data.split("|", 1)[1] if "|" in image_data else image_data
                                        delivery_content_lines.append(f"[图片] {image_url}")
                                    else:
                                        delivery_content_lines.append(payload_content)

                                db_manager.create_delivery_record(
                                    rule_id=delivery_payloads[0]['rule_id'],
                                    user_id=self.user_id,
                                    cookie_id=self.cookie_id,
                                    order_id=order_id,
                                    chat_id=chat_id,
                                    item_id=item_id,
                                    item_title=delivery_payloads[0].get('item_title') or item_title or item_id,
                                    buyer_id=send_user_id,
                                    card_id=delivery_payloads[0].get('card_id'),
                                    card_name=delivery_payloads[0].get('card_name'),
                                    card_type=delivery_payloads[0].get('card_type'),
                                    sent_count=sent_count,
                                    delivery_content='\n\n-----\n\n'.join(delivery_content_lines),
                                )
                                logger.info(f"自动发货明细记录已写入: 规则ID={delivery_payloads[0]['rule_id']}, 订单ID={order_id}")
                            except Exception as e:
                                logger.error(f"更新自动发货记录次数失败: {self._safe_str(e)}")
    ...
    async def refresh_token(self, captcha_retry_count: int = 0, token_expiry_retry_count: int = 0):
        ...
        try:
            ...
            logger.debug(f"【{self.cookie_id}】开始执行Cookie刷新任务")
            ...
            # 详细请求明细只在显式开启调试时输出，默认保留摘要日志。
            api_url = API_ENDPOINTS.get('token')
            if _is_verbose_token_refresh_logging_enabled():
                ...
                for key, value in sorted(params.items()):
                    if key == 'sign':
                        logger.debug(f"【{self.cookie_id}】  {key}: {value[:20]}...{value[-10:] if len(value) > 30 else value} (长度: {len(value)})")
                    else:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    response_summary = _summarize_token_refresh_response(response.status, res_json)
                    if _is_verbose_token_refresh_logging_enabled():
                        logger.debug(f"【{self.cookie_id}】--- API响应信息 ---")
                        logger.debug(f"【{self.cookie_id}】  状态码: {response.status}")
                        logger.debug(f"【{self.cookie_id}】  响应头: {dict(response.headers)}")
                        logger.debug(f"【{self.cookie_id}】  响应内容: {json.dumps(res_json, ensure_ascii=False, indent=2)}")
                        logger.debug(f"【{self.cookie_id}】================================")
                    else:
                        logger.info(f"【{self.cookie_id}】Token刷新接口返回: {response_summary}")
                    ...
                    if isinstance(res_json, dict):
                        ...
                        if any('SUCCESS::调用成功' in ret for ret in ret_value):
                            if 'data' in res_json and 'accessToken' in res_json['data']:
                                ...
                                logger.info(f"【{self.cookie_id}】Token刷新成功，已重置消息接收冷却状态")
                                # 标记为成功
                    ...
                    logger.error(f"【{self.cookie_id}】Token刷新失败: {response_summary}")
                    ...
                    if not notification_sent:
                        ...
                        if is_ws_connected:
                            ...
                            await self.send_token_refresh_notification(f"Token刷新失败: {response_summary}", "token_refresh_failed")
                    else:
    ...
    def _need_captcha_verification(self, res_json: dict) -> bool:
        ...
        try:
            ...
            response_summary = _summarize_token_refresh_response(None, res_json)
            log_captcha_event(self.cookie_id, "检查滑块验证响应", None, f"响应摘要: {response_summary}")
            ...
            for keyword in captcha_keywords:
                if keyword in error_msg:
                    logger.debug(f"【{self.cookie_id}】检测到需要滑块验证的关键词: {keyword}")
                    return True
            ...
            if isinstance(data, dict) and 'url' in data:
                ...
                if 'punish' in url or 'captcha' in url or 'validate' in url:
                    logger.debug(f"【{self.cookie_id}】检测到验证URL: {_truncate_log_text(url, 160)}")
                    return True
    ...
    async def _handle_captcha_verification(self, res_json: dict) -> str:
        ...
        try:
            logger.warning(f"【{self.cookie_id}】开始处理滑块验证")
            ...
            if not verification_url:
                logger.debug(f"【{self.cookie_id}】未找到验证URL，认为不需要滑块验证，返回正常")
                return None

            logger.debug(f"【{self.cookie_id}】验证URL: {_truncate_log_text(verification_url, 160)}")
        ...
        try:
            ...
            try:
                ...
                if not new_cookies_dict:
                    ...
                    return False
                logger.debug(f"【{self.cookie_id}】新cookies解析成功，包含 {len(new_cookies_dict)} 个字段")
            except Exception as parse_e:
            ...
            try:
                ...
                logger.debug(f"【{self.cookie_id}】当前cookies包含 {len(current_cookies_dict)} 个字段")
                ...
                if updated_fields:
                    logger.info(f"【{self.cookie_id}】Cookie字段已更新: {len(updated_fields)} 项")
                    logger.debug(f"【{self.cookie_id}】更新的cookie字段: {', '.join(updated_fields)}")
                else:
                    logger.debug(f"【{self.cookie_id}】没有cookie字段需要更新")

                # 重新组装cookies字符串
                merged_cookies_str = '; '.join([f"{k}={v}" for k, v in merged_cookies_dict.items()])
                logger.debug(f"【{self.cookie_id}】合并后cookies包含 {len(merged_cookies_dict)} 个字段")
                
                # 打印合并后的Cookie字段详情
                logger.debug(f"【{self.cookie_id}】========== 合并后Cookie字段详情 ==========")
                logger.debug(f"【{self.cookie_id}】Cookie字段数: {len(merged_cookies_dict)}")
                logger.debug(f"【{self.cookie_id}】Cookie字段列表:")
                for i, (key, value) in enumerate(merged_cookies_dict.items(), 1):
                    if len(str(value)) > 50:
                        logger.debug(f"【{self.cookie_id}】  {i:2d}. {key}: {str(value)[:30]}...{str(value)[-20:]} (长度: {len(str(value))})")
                    else:
                        logger.debug(f"【{self.cookie_id}】  {i:2d}. {key}: {value}")

                important_keys = ['unb', '_m_h5_tk', '_m_h5_tk_enc', 'cookie2', 't', 'sgcookie', 'cna']
                logger.debug(f"【{self.cookie_id}】关键字段检查:")
                for key in important_keys:
                    if key in merged_cookies_dict:
                        val = merged_cookies_dict[key]
                        logger.debug(f"【{self.cookie_id}】  ✅ {key}: {'存在' if val else '为空'} (长度: {len(str(val)) if val else 0})")
                    else:
                        logger.debug(f"【{self.cookie_id}】  ❌ {key}: 缺失")
                logger.debug(f"【{self.cookie_id}】==========================================")
    ...
    async def _try_password_login_refresh(self, trigger_reason: str = "令牌/Session过期"):
        ...
        try:
            ...
            if result:
                ...
                # 打印密码登录获取的Cookie字段详情
                logger.debug(f"【{self.cookie_id}】========== 密码登录Cookie字段详情 ==========")
                logger.debug(f"【{self.cookie_id}】Cookie字段数: {len(result)}")
                logger.debug(f"【{self.cookie_id}】Cookie字段列表:")
                ...
                logger.debug(f"【{self.cookie_id}】Cookie字符串格式: {new_cookies_str[:200]}..." if len(new_cookies_str) > 200 else f"【{self.cookie_id}】Cookie字符串格式: {new_cookies_str}")
    ...
    async def _verify_cookie_validity(self) -> dict:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"【{self.cookie_id}】已创建测试图片: {test_image_path}")
                ...
                if upload_result:
                    # 上传成功，Cookie有效
                    logger.debug(f"【{self.cookie_id}】✅ 图片上传API验证通过: 上传成功 ({upload_result[:50]}...)")
        ...
        # 汇总结果
        if result['valid']:
            logger.debug(f"【{self.cookie_id}】✅ Cookie验证通过: 所有关键API均可用")
        else:
    ...
    async def _restart_instance(self):
        ...
        """
        try:
            logger.debug(f"【{self.cookie_id}】准备重启实例")
            ...
            if cookie_manager:
                # 通过CookieManager重启实例
                logger.debug(f"【{self.cookie_id}】通过CookieManager重启实例")
                ...
                def trigger_restart():
                    ...
                    try:
                        ...
                        logger.debug(f"【{self.cookie_id}】实例重启请求已触发")
                    except Exception as e:
                ...
                logger.info(f"【{self.cookie_id}】已发送实例重启请求，当前实例即将退出")
    ...
    async def save_items_list_to_db(self, items_list):
        ...
        try:
            ...
            def extract_item_images(pic_info):
                image_urls = []
                if isinstance(pic_info, dict):
                    direct_url = pic_info.get('picUrl') or pic_info.get('url')
                    if direct_url:
                        image_urls.append(direct_url)
                    for key in ('picList', 'pics', 'images'):
                        ...
                        if isinstance(values, list):
                            for value in values:
                                if isinstance(value, str) and value.strip():
                                    ...
                                    candidate = value.get('picUrl') or value.get('url') or value.get('imageUrl')
                                    if candidate:
            ...
            for item in items_list:
                ...
                batch_data.append({
                    ...
                    'item_category': item.get('category_name') or str(item.get('category_id', '')),
                    'item_price': item.get('price_text', ''),
                    'item_detail': json.dumps(item_detail, ensure_ascii=False),
                    'item_status': str(item.get('item_status', '')),
                    'item_images': extract_item_images(item.get('pic_info', {})),
                })
    ...
    def extract_item_id_from_message(self, message):
        ...
        try:
            ...
            logger.debug(f"【{self.cookie_id}】未能从消息中提取到商品ID")
            return None
        ...
        try:
            ...
            if item_id:
                ...
                if item_default_reply and item_default_reply.get('enabled', False):
                    ...
                    reply_image_url = item_default_reply.get('reply_image_url') or item_default_reply.get('reply_image') or ''
                    ...
                    if item_default_reply.get('reply_once', False) and chat_id:
                        if db_manager.has_default_reply_record(self.cookie_id, chat_id, item_id):
                            logger.info(f"【{self.cookie_id}】商品 {item_id} 已对 chat_id {chat_id} 回复过，跳过（只回复一次）")
                            return None
    ...
    async def get_item_reply(self, send_user_name: str, send_user_id: str, send_message: str, item_id: str = None):
        """获取商品回复内容，支持每个客户只回复一次"""
        try:
            from db_manager import db_manager
            ...
            reply_content = item_reply.get('reply_content', '')
            if not reply_content or not reply_content.strip():
                logger.info(f"【{self.cookie_id}】商品 {item_id} 回复内容为空，不进行回复")
                return "EMPTY_REPLY"

            reply_once = bool(item_reply.get('reply_once', False))
            if reply_once and send_user_id:
                if db_manager.has_item_reply_once_record(self.cookie_id, item_id, send_user_id):
                    logger.info(
                        f"【{self.cookie_id}】商品 {item_id} 已对客户 {send_user_id} 回复过，跳过（只回复一次）"
                    )
            ...
            try:
                ...
                logger.error(f"商品回复变量替换失败: {self._safe_str(format_error)}")
                formatted_reply = reply_content
            ...
            logger.error(f"获取商品回复失败: {self._safe_str(e)}")
            return None
    ...
    async def _update_default_reply_image_url(self, new_image_url: str, item_id: str = None):
        """更新默认回复的图片URL为CDN URL"""
        try:
            from db_manager import db_manager
            success = db_manager.update_default_reply_image_url(self.cookie_id, new_image_url, item_id)
            if success:
    ...
    async def _auto_delivery(self, item_id: str, item_title: str = None, order_id: str = None, send_user_id: str = None):
        ...
        try:
            ...
            if is_multi_spec:
                # 多规格商品：只匹配多规格发货规则
                if spec_name and spec_value:
                    logger.info(f"多规格商品，优先按商品绑定匹配发货规则: item_id={item_id} [{spec_name}:{spec_value}]")
                    delivery_rules = db_manager.get_delivery_rules_by_item_id_and_spec(item_id, spec_name, spec_value)
                    delivery_rules = [r for r in delivery_rules if r.get('is_multi_spec')]

                    if not delivery_rules:
                        logger.info(f"未找到商品绑定规则，回退关键字匹配: {search_text[:50]}... [{spec_name}:{spec_value}]")
                ...
                # 非多规格商品：只匹配非多规格发货规则
                logger.info(f"非多规格商品，优先按商品绑定匹配发货规则: item_id={item_id}")
                delivery_rules = db_manager.get_delivery_rules_by_item_id(item_id)
                delivery_rules = [r for r in delivery_rules if not r.get('is_multi_spec')]

                if not delivery_rules:
                    logger.info(f"未找到商品绑定规则，回退关键字匹配: {search_text[:50]}...")
            ...
            if order_id:
                ...
                if delivery_content:
                    ...
                    logger.info(f"自动发货内容准备成功: 规则ID={rule['id']}, 内容长度={len(final_content)}")
                    return {
                        'rule_id': rule['id'],
                        'card_id': rule['card_id'],
                        'card_name': rule['card_name'],
                        'card_type': rule['card_type'],
                        'item_title': item_title_for_save or search_text or item_id,
    ...
    async def init(self, ws):
        ...
        if not self.current_token:
            logger.warning(f"【{self.cookie_id}】初始化失败：无法获取有效token")
            ...
            if not token_refresh_attempted:
                ...
                logger.debug(f"【{self.cookie_id}】由于刚刚尝试过token刷新，跳过重复的初始化失败通知")
    ...
    async def heartbeat_loop(self, ws):
        ...
        logger.info(f"【{self.cookie_id}】已进入心跳阶段，开始等待心跳收发")
    ...
    async def handle_heartbeat_response(self, message_data):
        ...
        try:
            ...
            if lwp == "/!" and code == 200:
                ...
                logger.debug(f"【{self.cookie_id}】心跳响应正常")
                return True
            if lwp == "/!":
                logger.warning(f"【{self.cookie_id}】收到心跳响应但状态异常: lwp=/!, code={code}")
                return False
            if code == 200:
                logger.debug(f"【{self.cookie_id}】忽略非心跳的200响应: lwp={lwp}")
        except Exception as e:
    ...
    async def pause_cleanup_loop(self):
        ...
        try:
            ...
            # 确保任务能正常结束
            logger.debug(f"【{self.cookie_id}】清理循环已退出")
    ...
    async def cookie_refresh_loop(self):
        ...
        try:
            ...
            # 确保任务能正常结束
            logger.debug(f"【{self.cookie_id}】Cookie刷新循环已退出")
    ...
    async def _execute_cookie_refresh(self, current_time):
        ...
        async with self.cookie_refresh_lock:
            try:
                logger.debug(f"【{self.cookie_id}】开始Cookie刷新任务，暂时暂停心跳以避免连接冲突...")
                ...
                if success:
                    ...
                    logger.debug(f"【{self.cookie_id}】Cookie刷新任务完成，心跳已恢复")
                    
                    # 刷新成功后，验证Cookie有效性
                    logger.debug(f"【{self.cookie_id}】开始验证刷新后的Cookie有效性...")
                    try:
                        ...
                        if not validation_result['valid']:
                            ...
                            logger.info(
                                f"【{self.cookie_id}】Cookie刷新成功并验证通过: "
                                f"{_truncate_log_text(validation_result['details'], 160)}"
                            )
                ...
                if (self.ws and not self.ws.closed and
                    ...
                    logger.debug(f"【{self.cookie_id}】Cookie刷新完成，心跳任务正常运行")
                    self.heartbeat_task = asyncio.create_task(self.heartbeat_loop(self.ws))
    ...
    async def refresh_cookies_from_qr_login(self, qr_cookies_str: str, cookie_id: str = None, user_id: int = None):
        ...
        try:
            ...
            logger.debug(f"【{target_cookie_id}】========== 扫码登录真实Cookie字段详情 ==========")
            logger.debug(f"【{target_cookie_id}】Cookie字段数: {len(real_cookies_dict)}")
            logger.debug(f"【{target_cookie_id}】Cookie字段列表:")
            for i, (key, value) in enumerate(real_cookies_dict.items(), 1):
                if len(str(value)) > 50:
                    logger.debug(f"【{target_cookie_id}】  {i:2d}. {key}: {str(value)[:30]}...{str(value)[-20:]} (长度: {len(str(value))})")
                else:
                    logger.debug(f"【{target_cookie_id}】  {i:2d}. {key}: {value}")
            
            # 检查关键字段
            important_keys = ['unb', '_m_h5_tk', '_m_h5_tk_enc', 'cookie2', 't', 'sgcookie', 'cna']
            logger.debug(f"【{target_cookie_id}】关键字段检查:")
            for key in important_keys:
                if key in real_cookies_dict:
                    val = real_cookies_dict[key]
                    logger.debug(f"【{target_cookie_id}】  ✅ {key}: {'存在' if val else '为空'} (长度: {len(str(val)) if val else 0})")
                else:
                    logger.debug(f"【{target_cookie_id}】  ❌ {key}: 缺失")
            logger.debug(f"【{target_cookie_id}】==========================================")

            # 打印完整的真实Cookie内容
            logger.debug(f"【{target_cookie_id}】=== 完整真实Cookie内容 ===")
            logger.debug(f"【{target_cookie_id}】Cookie字符串长度: {len(real_cookies_str)}")
            logger.debug(f"【{target_cookie_id}】Cookie完整内容:")
            logger.debug(f"【{target_cookie_id}】{real_cookies_str}")

            # 打印所有Cookie字段的详细信息
            logger.debug(f"【{target_cookie_id}】=== Cookie字段详细信息 ===")
    ...
    async def _refresh_cookies_via_browser_page(self, current_cookies_str: str):
        ...
        try:
            ...
            logger.debug(f"【{self.cookie_id}】真实Cookie: {real_cookies_str}")
            # 检查关键字段
            important_keys = ['unb', '_m_h5_tk', '_m_h5_tk_enc', 'cookie2', 't', 'sgcookie', 'cna']
            logger.debug(f"【{self.cookie_id}】关键字段检查:")
            for key in important_keys:
        ...
        try:
            ...
            if triggered_by_refresh_token:
                ...
                try:
                    ...
                    # ⚠️ _restart_instance() 已触发重启，当前任务即将被取消
                    # 不要等待或执行耗时操作
                    logger.debug(f"【{self.cookie_id}】重启请求已触发(via _refresh_cookies_via_browser)")
    ...
    async def _create_websocket_connection(self, headers):
        ...
        if _is_verbose_websocket_logging_enabled():
    ...
    def _get_query_param_from_url(self, url: str, param_name: str) -> str:
        """从 URL 中提取指定查询参数，失败时返回空字符串。"""
        try:
    ...
    async def _process_chat_message_reply(self, message_data: dict, websocket, send_user_name: str,
        ...
        try:
            ...
            item_reply_once_record = None

            # 构造用户URL
            user_url = f'https://www.goofish.com/personal?userId={send_user_id}'

            reply = None
            item_reply_result = await self.get_item_reply(send_user_name, send_user_id, send_message, item_id)
            if item_reply_result == "EMPTY_REPLY":
                logger.info(f"[{msg_time}] 【{self.cookie_id}】商品回复内容为空，跳过自动回复")
                return
            elif item_reply_result == "ALREADY_REPLIED":
                return
            elif item_reply_result and isinstance(item_reply_result, dict):
                reply = item_reply_result.get('text')
                reply_source = '商品回复'
                if item_reply_result.get('reply_once', False):
                    item_reply_once_record = item_reply_result.get('item_id')

            # 判断是否启用API回复
            if not reply and AUTO_REPLY.get('api', {}).get('enabled', False):
                                        ...
                                        if os.path.exists(local_image_path):
                                            ...
                                            async with uploader:
                                                ...
                                                if cdn_url:
                                                    ...
                                                    # 更新数据库中的图片URL为CDN URL
                                                    default_reply_item_id = (
                                                        default_reply_result.get('item_id')
                                                        if default_reply_result.get('scope') == 'item'
                                                        else None
                                                    )
            ...
            if reply:
                ...
                if reply.startswith("__IMAGE_SEND__"):
                    ...
                    if item_reply_once_record and send_user_id:
                        db_manager.add_item_reply_once_record(self.cookie_id, item_reply_once_record, send_user_id)
                    # 记录发出的消息
                    msg_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        ...
        try:
            ...
            try:
                ...
                if order_id:
                    ...
                    logger.debug(f"【{self.cookie_id}】当前消息未携带订单ID")
            except Exception as e:
                logger.error(f"【{self.cookie_id}】提取订单ID失败: {self._safe_str(e)}")
            ...
            try:
                ...
                if not item_id:
                    ...
                    logger.info(f"【{self.cookie_id}】未提取到商品ID，使用临时ID: {item_id}")
            ...
            if not self.is_chat_message(message):
                logger.debug(f"【{self.cookie_id}】忽略非聊天消息")
                return
            ...
            peer_user_id = self._get_query_param_from_url(reminder_url, "peerUserId")
            same_as_self = send_user_id == self.myid
            same_as_peer = bool(peer_user_id) and send_user_id == peer_user_id

            # 记录关键方向判断信息，便于排查误判
            logger.info(
                f"[{msg_time}] 【{self.cookie_id}】消息方向判定: "
                f"senderUserId={send_user_id}, myid={self.myid}, peerUserId={peer_user_id or '-'}"
            )

            # 判断消息方向
            # 只有在 senderUserId 明确等于当前账号，且不等于会话对端用户时，才认为是自己手动发出。
            # 这样可以避免某些异常同步包把买家消息误判成手动消息，导致自动回复被暂停。
            if same_as_self and not same_as_peer:
                logger.info(f"[{msg_time}] 【手动发出】 商品({item_id}): {send_message}")
                return
            else:
                if same_as_self and same_as_peer:
                    logger.warning(
                        f"[{msg_time}] 【{self.cookie_id}】检测到 senderUserId 与 myid 相同，"
                        f"且与 peerUserId 也相同，这是当前账号的镜像会话消息，跳过自动回复"
                    )
                    return
    ...
    async def main(self):
        """主程序入口"""
        try:
            logger.debug(f"【{self.cookie_id}】开始启动XianyuLive主程序")
            await self.create_session()  # 创建session
            logger.debug(f"【{self.cookie_id}】Session创建完成，开始WebSocket连接循环")
            ...
            while True:
                try:
                    ...
                    if _is_verbose_websocket_logging_enabled():
                        logger.debug(f"【{self.cookie_id}】WebSocket目标地址: {self.base_url}")
                    ...
                    async with await self._create_websocket_connection(headers) as websocket:
                        ...
                        try:
                            # 开始初始化
                            await self.init(websocket)
                            logger.debug(f"【{self.cookie_id}】WebSocket初始化完成！")
                    ...
                    error_summary = _truncate_log_text(error_msg, 200)
                    if is_connection_closed:
                        logger.warning(
                            f"【{self.cookie_id}】WebSocket连接已关闭，准备重连 "
                            f"({self.connection_failures}/{self.max_connection_failures}) - {error_summary}"
                        )
                    else:
                        logger.error(
                            f"【{self.cookie_id}】WebSocket连接异常，准备重连 "
                            f"({self.connection_failures}/{self.max_connection_failures}) - "
                            f"{error_type}: {error_summary}"
                        )
                        _log_startup_retry_traceback(self.cookie_id, "WebSocket连接异常")
                    ...
                    if self.connection_failures >= self.max_connection_failures:
                        ...
                        # 如果密码登录刷新失败或异常，则重启实例
                        logger.warning(f"【{self.cookie_id}】准备重启实例")
                        self.connection_failures = 0  # 重置失败计数
                        
                        # 先清理后台任务，避免与重启过程冲突
                        logger.debug(f"【{self.cookie_id}】重启前先清理后台任务")
                        try:
                            await asyncio.wait_for(
                                self._cancel_background_tasks(),
                                timeout=8.0  # 给足够时间让任务响应
                            )
                            logger.debug(f"【{self.cookie_id}】后台任务已清理完成")
                        except asyncio.TimeoutError:
                        ...
                        logger.info(f"【{self.cookie_id}】重启请求已触发，新实例将自动启动")
                    ...
                    try:
                        ...
                        # 这样可以避免等待任务取消导致的阻塞问题
                        logger.debug(f"【{self.cookie_id}】准备重置后台任务引用（快速重连模式）")
                        self._reset_background_tasks()
                        logger.debug(f"【{self.cookie_id}】后台任务引用已重置，可以立即重连")

                        # 等待后重试 - 使用可中断的sleep，并定期输出日志证明进程还活着
                        logger.debug(f"【{self.cookie_id}】开始等待 {retry_delay} 秒")
                        # 强制刷新日志缓冲区，确保日志被写入
                        try:
                            sys.stdout.flush()
                        except:
                        ...
                        while remaining > 0:
                            ...
                            try:
                                ...
                                _log_startup_retry_traceback(self.cookie_id, "等待期间异常")
                                # 即使出错也继续等待剩余时间
                                if remaining > 0:
                        ...
                        _log_startup_retry_traceback(self.cookie_id, "重连清理异常")
                        ...
                        # 使用可中断的sleep，并定期输出日志
                        logger.debug(f"【{self.cookie_id}】清理失败后开始等待 {retry_delay} 秒")
                        chunk_size = 5.0
                        remaining = retry_delay
                        start_time = time.time()
                        ...
                        while remaining > 0:
                            ...
                            try:
                                ...
                                remaining -= sleep_time
                                if remaining > 0:
                                    logger.debug(f"【{self.cookie_id}】清理失败后等待中，剩余 {remaining:.1f} 秒")
                            except asyncio.CancelledError:
                        ...
                        logger.debug(f"【{self.cookie_id}】清理失败后等待完成（总耗时 {time.time() - start_time:.1f} 秒）")
            ...
            # 清空当前token
            if self.current_token:
                logger.debug(f"【{self.cookie_id}】程序退出，清空当前token")
                self.current_token = None
            ...
            if has_pending_tasks:
                logger.debug(f"【{self.cookie_id}】检测到未完成的后台任务，执行清理")
                # 使用统一的任务清理方法，添加超时保护
                try:
                    await asyncio.wait_for(
                        self._cancel_background_tasks(),
                ...
                logger.debug(f"【{self.cookie_id}】所有后台任务已清理完成，跳过重复清理")
                # 确保任务引用被重置
                self.heartbeat_task = None
                self.token_refresh_task = None
                self.cleanup_task = None
            ...
            if self.background_tasks:
                logger.debug(f"【{self.cookie_id}】等待 {len(self.background_tasks)} 个后台任务完成")
                try:
                    await asyncio.wait_for(
            ...
            logger.debug(f"【{self.cookie_id}】XianyuLive主程序已完全退出")
                    ...
                    for card in card_list:
                        ...
                        if card_data:
                            ...
                            item_info = {
                                ...
                                'seller_nick': (
                                    card_data.get('userNickName')
                                    or card_data.get('sellerNick')
                                    or card_data.get('sellerNickName')
                                    or card_data.get('nick')
                                    or ''
                                ),
                                ...
                                'category_name': (
                                    card_data.get('categoryName')
                                    or card_data.get('categoryText')
                                    or card_data.get('catName')
                                    or card_data.get('cateName')
                                    or ''
                                ),
    ...
    def _build_goofish_playwright_cookies(self):
        """将当前 Cookie 字符串转换为 Playwright 可用格式"""
        cookies = []
        for cookie_pair in self.cookies_str.split('; '):
            ...
            for domain in ('.goofish.com', 'www.goofish.com'):
                cookies.append({
                    'name': cookie_name,
                    'value': cookie_value,
                    'domain': domain,
    ...
    async def _click_action_by_labels(self, scope, labels):
        """按候选文案点击按钮或链接，返回命中的文案"""
        for label in labels:
            label_regex = re.compile(re.escape(label))
            candidates = [
                scope.get_by_role("button", name=label_regex),
                scope.get_by_role("link", name=label_regex),
            ...
            for locator in candidates:
                ...
                try:
                    ...
                    logger.info(f"【{self.cookie_id}】浏览器动作命中控件: {label}")
                    return label
                except Exception:
                    continue
        return None

    async def _complete_relist_publish_flow(self, page):
        """处理重新上架后的确认发布流程"""
        confirm_labels = ['确认上架', '继续上架', '确认发布', '立即发布', '发布', '确认']
        clicked_label = await self._click_action_by_labels(page, confirm_labels)
        if clicked_label:
            logger.info(f"【{self.cookie_id}】重新上架确认流程已点击: {clicked_label}")
            await asyncio.sleep(1.5)
        return clicked_label

    async def _execute_item_action_on_page(self, page, item_id: str, operation: str, item_title: str = "") -> Dict[str, Any]:
        """在当前页面尝试执行商品动作"""
        action_labels = {
            'polish': ['擦亮', '立即擦亮', '一键擦亮'],
            'relist': ['重新上架', '立即上架', '重新发布', '上架', '发布'],
        }
        confirm_labels = {
            'polish': ['确认擦亮', '立即擦亮', '确认', '确定'],
            'relist': ['确认上架', '继续上架', '确认', '确定'],
        }

        scope_candidates = []
        selector_candidates = [
            f'[data-itemid="{item_id}"]',
        ...
        for selector in selector_candidates:
            ...
            try:
                if await locator.count() > 0:
                    scope_candidates.append(locator.first)
            except Exception:
                continue

        if item_title:
            try:
                title_locator = page.get_by_text(item_title.strip(), exact=False)
                if await title_locator.count() > 0:
                    scope_candidates.append(title_locator.first)
            except Exception:
                pass

        scope_candidates.append(page)

        for scope in scope_candidates:
            clicked_label = await self._click_action_by_labels(scope, action_labels.get(operation, []))
            if not clicked_label:
                continue

            await asyncio.sleep(1)
            await self._click_action_by_labels(page, confirm_labels.get(operation, []))

            if operation == 'relist':
                await self._complete_relist_publish_flow(page)

            return {
                'success': True,
                'message': f'商品{item_id}已执行{clicked_label}',
                'matched_label': clicked_label,
            }

        return {
            'success': False,
            'message': f'页面中未找到商品 {item_id} 的{operation}入口',
        }

    async def execute_item_management_action(self, item_id: str, operation: str, item_title: str = "", show_browser: bool = False) -> dict:
        """通过浏览器真实执行商品擦亮或上架"""
        if operation not in {'polish', 'relist'}:
            return {'success': False, 'message': f'不支持的商品操作: {operation}'}

        playwright = None
        browser = None
        page = None
        operation_name = '擦亮' if operation == 'polish' else '上架'
        candidate_urls = [
            f'https://www.goofish.com/item?id={item_id}',
            'https://www.goofish.com/personal',
            'https://www.goofish.com/personal?spm=a21ybx.personal.publish.0.0#/publish',
            'https://www.goofish.com/idle',
            'https://www.goofish.com/idle?spm=a21ybx.personal.publish.0.0#/publish',
        ]
        ...
        try:
            ...
            context = await browser.new_context(
                viewport={'width': 1920, 'height': 1080},
                locale='zh-CN',
                user_agent=DEFAULT_HEADERS.get('User-Agent') or DEFAULT_HEADERS.get('user-agent') or
                'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/138.0.0.0 Safari/537.36'
            )
            await context.add_cookies(self._build_goofish_playwright_cookies())
            page = await context.new_page()

            for target_url in candidate_urls:
                try:
                    logger.info(f"【{self.cookie_id}】尝试通过浏览器执行商品{operation_name}: {target_url}")
                    await page.goto(target_url, wait_until='domcontentloaded', timeout=45000)
                    await asyncio.sleep(1.5)
                    result = await self._execute_item_action_on_page(page, item_id, operation, item_title=item_title)
                    if result.get('success'):
                        result['executed_by'] = 'browser'
                        result['page_url'] = page.url
                        return result
                except Exception as page_error:
                    logger.warning(f"【{self.cookie_id}】页面尝试失败({target_url}): {self._safe_str(page_error)}")
                    continue

            return {
                'success': False,
                'message': f'未找到商品 {item_id} 的真实{operation_name}入口，请检查账号登录态或页面结构是否变化',
                'executed_by': 'browser',
            }
        except Exception as e:
            logger.error(f"【{self.cookie_id}】浏览器执行商品{operation_name}失败: {self._safe_str(e)}")
            return {
                'success': False,
                'message': f'浏览器执行商品{operation_name}失败: {self._safe_str(e)}',
                'executed_by': 'browser',
            }
        finally:
            try:
