...
from embedded_netdisk_app import app as embedded_netdisk_app
from routes import account_routes, auth_routes, order_routes
...
def _looks_like_account_id(value: str) -> bool:
    ...
    if not value or _contains_cjk(value):
        return False
    lowered = value.lower()
    if lowered.startswith(("tb", "xy", "cn", "user", "nick")) and any(ch.isdigit() for ch in lowered):
        return True
    if value.isdigit():
        return True
    if len(value) >= 6 and sum(ch.isdigit() for ch in value) >= 3:
        return True
    return False


def _extract_best_nickname_from_json(obj: Any) -> str:
    candidates: List[Tuple[int, str]] = []

    def add_candidate(key: str, value: str):
        ...
        if _contains_cjk(value):
            score += 100
        if "xianyu" in key_lower or "idle" in key_lower or "goofish" in key_lower:
            score += 60
        if "nick" in key_lower:
            score += 40
        if "display" in key_lower:
            score += 35
        if "show" in key_lower:
            score += 25
        if "name" in key_lower:
            score += 10
        if 2 <= len(value) <= 24:
            score += 10
        if value.startswith("xy") or value.startswith("tb"):
            score -= 20
        if _looks_like_account_id(value):
            score -= 80
    ...
    def walk(x: Any):
        if isinstance(x, dict):
            for k, v in x.items():
                if isinstance(v, str):
                    add_candidate(str(k), v)
                    trimmed = v.strip()
                    if (trimmed.startswith("{") and trimmed.endswith("}")) or (
                        trimmed.startswith("[") and trimmed.endswith("]")
                    ):
                        try:
                            walk(json.loads(trimmed))
                        except Exception:
                            pass
                else:
                    walk(v)
    ...
    for best_score, best_value in candidates:
        if best_score >= 60 and not _looks_like_account_id(best_value):
            return best_value
    return ""


def _extract_best_avatar_url_from_json(obj: Any) -> str:
    candidates: List[Tuple[int, str]] = []
    ...
    def add_candidate(key: str, value: str):
        ...
        key_lower = key.lower()
        score = 0
        if "avatar" in key_lower or "head" in key_lower or "profile" in key_lower or "icon" in key_lower:
            score += 80
        if "url" in key_lower or "image" in key_lower or "img" in key_lower or "pic" in key_lower:
            score += 30
        if "xianyu" in key_lower or "idle" in key_lower or "goofish" in key_lower:
            score += 10
        if "qrcode" in key_lower:
            score -= 60
        if value.startswith("//"):
            score += 5
        if len(value) < 300:
    ...
    if best_value.startswith("//"):
        return "https:" + best_value
    return best_value


def fetch_xianyu_profile_from_headinfo(cookie_value: str) -> Tuple[str, str]:
    if not cookie_value:
        return ("", "")
    ...
    url = API_ENDPOINTS.get("message_headinfo")
    if not url:
        return ("", "")

    try:
        cookies_dict = trans_cookies(cookie_value)
    except Exception:
        return ("", "")
    ...
    def _drop_cookie_by_name(name: str):
        try:
    ...
    def _read_cookie_value(name: str) -> str:
        try:
            for c in reversed(list(session.cookies)):
                if getattr(c, "name", None) == name:
            ...
            pass
        return str(cookies_dict.get(name) or "")

    def _ensure_h5_token(force_refresh: bool = False) -> str:
        if force_refresh:
            _drop_cookie_by_name("_m_h5_tk")
            _drop_cookie_by_name("_m_h5_tk_enc")
            _drop_cookie_by_name("m_h5_tk")
            _drop_cookie_by_name("m_h5_tk_enc")

        token_raw = _read_cookie_value("_m_h5_tk") or _read_cookie_value("m_h5_tk") or ""
        token_raw = str(token_raw or "").strip()
        token = token_raw.split("_")[0] if "_" in token_raw else token_raw
        if token:
            return token
        ...
        try:
            session.get(seed_url, timeout=6)
        except Exception:
            pass
        ...
        try:
            session.post(seed_url, params=params_seed, data={"data": data_seed}, timeout=6)
        except Exception:
            pass
        token_raw = _read_cookie_value("_m_h5_tk") or _read_cookie_value("m_h5_tk") or ""
        token_raw = str(token_raw or "").strip()
    ...
    def _read_h5_token() -> str:
        return _ensure_h5_token(force_refresh=False)

    token = _ensure_h5_token(force_refresh=False)
    if not token:
        return ("", "")

    t_value = str(int(time.time() * 1000))
    data_val = "{}"
    params = {
        "jsv": "2.7.2",
        "appKey": "34839810",
        "t": t_value,
        "sign": generate_sign(t_value, token, data_val),
        "v": "1.0",
        "type": "originaljson",
        "accountSite": "xianyu",
        "dataType": "json",
    ...
    from utils.xianyu_utils import is_token_expired_error

    def _call() -> Dict[str, Any]:
        params["t"] = str(int(time.time() * 1000))
        params["sign"] = generate_sign(params["t"], _read_h5_token(), data_val)
        response = session.post(url, params=params, data={"data": data_val}, timeout=6)
    ...
    try:
        ...
        if is_token_expired_error(ret_value) or ("FAIL_SYS_TOKEN_EMPTY" in ret_str) or ("令牌为空" in ret_str):
            _ensure_h5_token(force_refresh=True)
            res_json = _call()
    except Exception:
        return ("", "")

    nickname = _extract_best_nickname_from_json(res_json)
    avatar_url = _extract_best_avatar_url_from_json(res_json)
    return (nickname, avatar_url)


def fetch_xianyu_profile_debug_from_headinfo(cookie_value: str) -> Dict[str, Any]:
    if not cookie_value:
        return {"success": False, "message": "cookie为空"}

    try:
        from config import API_ENDPOINTS, DEFAULT_HEADERS
        import requests
    except Exception as e:
        return {"success": False, "message": f"依赖加载失败: {str(e)}"}

    url = API_ENDPOINTS.get("message_headinfo")
    if not url:
        return {"success": False, "message": "未配置 message_headinfo 接口"}

    try:
        cookies_dict = trans_cookies(cookie_value)
    except Exception as e:
        return {"success": False, "message": f"Cookie解析失败: {str(e)}"}
    ...
    token = _ensure_h5_token(force_refresh=False)
    if not token:
        return {"success": False, "message": "Cookie中缺少 _m_h5_tk / m_h5_tk 且无法自动获取"}
    ...
    try:
        ...
        if is_token_expired_error(ret_value):
            token = _ensure_h5_token(force_refresh=True)
            params["t"] = str(int(time.time() * 1000))
            ...
            ret_str = str(ret_value)
            if ("FAIL_SYS_TOKEN_EMPTY" in ret_str) or ("令牌为空" in ret_str):
                token = _ensure_h5_token(force_refresh=True)
                params["t"] = str(int(time.time() * 1000))
                params["sign"] = generate_sign(params["t"], token, data_val)
                response = session.post(url, params=params, data={"data": data_val}, timeout=6)
        ...
        return {"success": False, "message": f"调用headinfo失败: {str(e)}"}
...
for _cidr in [p.strip() for p in LOCAL_ONLY_ALLOW_CIDR.split(",") if p.strip()]:
    try:
        _LOCAL_ONLY_NETWORKS.append(ipaddress.ip_network(_cidr, strict=False))
    except Exception:
        pass
...
def verify_token(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Optional[Dict[str, Any]]:
    return api_auth.verify_token(credentials)


def verify_admin_token(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Dict[str, Any]:
    return api_auth.verify_admin_token(credentials)


def require_auth(user_info: Optional[Dict[str, Any]] = Depends(verify_token)):
    return api_auth.require_auth(user_info)


def get_current_user(user_info: Dict[str, Any] = Depends(require_auth)) -> Dict[str, Any]:
    return api_auth.get_current_user(user_info)


def get_current_user_optional(user_info: Optional[Dict[str, Any]] = Depends(verify_token)) -> Optional[Dict[str, Any]]:
    return api_auth.get_current_user_optional(user_info)


def get_user_log_prefix(user_info: Dict[str, Any] = None) -> str:
    return api_auth.get_user_log_prefix(user_info)


def require_admin(current_user: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    return api_auth.require_admin(current_user)


def _can_access_cookie(cookie_id: str, current_user: Dict[str, Any]) -> bool:
    return api_auth.can_access_cookie(cookie_id, current_user)


def _ensure_cookie_access(cookie_id: str, current_user: Dict[str, Any], action: str = "访问") -> None:
    api_auth.ensure_cookie_access(cookie_id, current_user, action)


def _get_qr_login_instance(cookie_id: str):
    from XianyuAutoAsync import XianyuLive
    return XianyuLive.get_instance(cookie_id)


def log_with_user(level: str, message: str, user_info: Dict[str, Any] = None):
    api_auth.log_with_user(level, message, user_info)
...
async def local_only_guard(request, call_next):
    ...
    for net in _LOCAL_ONLY_NETWORKS:
        if client_ip in net:
            return await call_next(request)
...
async def login(request: LoginRequest):
    return await auth_routes.login(request)


# 验证token接口
@app.get('/verify')
async def verify(user_info: Optional[Dict[str, Any]] = Depends(verify_token)):
    return await auth_routes.verify(user_info)


# 登出接口
@app.post('/logout')
async def logout(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)):
    return await auth_routes.logout(credentials)
...
async def send_verification_code(request: SendCodeRequest):
    return await auth_routes.send_verification_code(request)


# 用户注册接口
@app.post('/register')
async def register(request: RegisterRequest):
    return await auth_routes.register(request)
...
def verify_api_key(api_key: str) -> bool:
    ...
    try:
        # 从系统设置中获取QQ回复消息秘钥
        from db_manager import db_manager
        qq_secret_key = (db_manager.get_system_setting('qq_reply_secret_key') or "").strip()
        env_secret_key = os.getenv("SEND_MESSAGE_API_KEY", "").strip()
        effective_secret = qq_secret_key or env_secret_key

        if not effective_secret:
            logger.warning("发送消息接口未配置API秘钥，拒绝请求")
            return False

        return api_key == effective_secret
    except Exception as e:
...
async def xianyu_reply(req: RequestModel):
    ...
    is_item_default_reply = False

    if not msg_template:
        # 从数据库获取默认回复
        from db_manager import db_manager
        item_default_reply_settings = (
            db_manager.get_item_default_reply(req.cookie_id, req.item_id) if req.item_id else None
        )

        if item_default_reply_settings and item_default_reply_settings.get('enabled', False):
            if item_default_reply_settings.get('reply_once', False):
                if db_manager.has_default_reply_record(req.cookie_id, req.chat_id, req.item_id):
                    raise HTTPException(status_code=404, detail="该对话已使用该商品的默认回复，不再重复回复")

            candidate_template = item_default_reply_settings.get('reply_content', '') or ''
            if candidate_template:
                msg_template = candidate_template
                is_default_reply = True
                is_item_default_reply = True

        if not msg_template:
            default_reply_settings = db_manager.get_default_reply(req.cookie_id)
        else:
            default_reply_settings = None
        ...
        if default_reply_settings and default_reply_settings.get('enabled', False):
            ...
            candidate_template = default_reply_settings.get('reply_content', '') or ''
            if candidate_template:
                msg_template = candidate_template
                is_default_reply = True
                is_item_default_reply = False
    ...
    if is_default_reply:
        from db_manager import db_manager
        if is_item_default_reply:
            item_default_reply_settings = db_manager.get_item_default_reply(req.cookie_id, req.item_id)
            if item_default_reply_settings and item_default_reply_settings.get('reply_once', False):
                db_manager.add_default_reply_record(req.cookie_id, req.chat_id, req.item_id)
        else:
            default_reply_settings = db_manager.get_default_reply(req.cookie_id)
...
def list_cookies(current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.list_cookies(current_user)


@app.get("/cookies/details")
def get_cookies_details(current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_cookies_details(
        current_user,
        _xianyu_nickname_fetch_limiter,
        _looks_like_account_id,
        fetch_xianyu_profile_from_headinfo,
    )


@app.post("/cookies")
def add_cookie(item: CookieIn, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.add_cookie(item, current_user)
...
def update_cookie_login_info(cid: str, update_data: AccountLoginInfoUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.update_cookie_login_info(cid, update_data, current_user)
...
def update_cookie(cid: str, item: CookieIn, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.update_cookie(cid, item, current_user)
...
def update_cookie_account_info(cid: str, info: CookieAccountInfo, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.update_cookie_account_info(cid, info, current_user)


@app.get("/cookie/{cid}/details")
def get_cookie_account_details(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_cookie_account_details(cid, current_user)


@app.get("/cookie/{cid}/headinfo-debug")
def get_cookie_headinfo_debug(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_cookie_headinfo_debug(
        cid,
        current_user,
        fetch_xianyu_profile_debug_from_headinfo,
    )


@app.post("/cookie/{cid}/refresh-profile")
def refresh_cookie_profile(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.refresh_cookie_profile(cid, current_user, fetch_xianyu_profile_from_headinfo)
...
async def reset_qr_cookie_refresh_cooldown(
    ...
    try:
        ...
        _ensure_cookie_access(cookie_id, current_user, "操作")

        # 如果有对应的活跃实例，直接重置
        instance = _get_qr_login_instance(cookie_id)
        if instance:
...
async def get_qr_cookie_refresh_cooldown_status(
    ...
    try:
        ...
        _ensure_cookie_access(cookie_id, current_user, "访问")
...
def _parse_notification_channel_config(config: Any) -> Dict[str, Any]:
...
async def _send_test_dingtalk_notification(config_data: Dict[str, Any], message: str) -> None:
    import aiohttp
    import hmac
    import base64

    webhook_url = (config_data.get("webhook_url") or config_data.get("config") or "").strip()
    secret = (config_data.get("secret") or "").strip()
    if not webhook_url:
        raise ValueError("钉钉 Webhook 地址不能为空")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(webhook_url, json=payload, timeout=10) as response:
            ...
            if response.status != 200 or data.get("errcode") not in (0, "0", None):
                raise ValueError(data.get("errmsg") or f"钉钉通知发送失败: HTTP {response.status}")


async def _send_test_feishu_notification(config_data: Dict[str, Any], message: str) -> None:
    import aiohttp
    import hmac
    import base64
    ...
    secret = (config_data.get("secret") or "").strip()
    if not webhook_url:
        raise ValueError("飞书 Webhook 地址不能为空")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(webhook_url, json=payload, timeout=10) as response:
            data = await response.json(content_type=None)
            if response.status != 200 or data.get("code") not in (0, "0", None):
                raise ValueError(data.get("msg") or f"飞书通知发送失败: HTTP {response.status}")


async def _send_test_bark_notification(config_data: Dict[str, Any], message: str) -> None:
    import aiohttp

    device_key = (config_data.get("device_key") or "").strip()
    if not device_key:
        raise ValueError("Bark 设备密钥不能为空")

    server_url = (config_data.get("server_url") or "https://api.day.app").rstrip("/")
    payload = {
        "device_key": device_key,
        "title": config_data.get("title") or "闲鱼自动回复测试通知",
        "body": message,
        "sound": config_data.get("sound") or "default",
        "group": config_data.get("group") or "xianyu",
    }
    if config_data.get("icon"):
        payload["icon"] = config_data["icon"]
    if config_data.get("url"):
        payload["url"] = config_data["url"]

    async with aiohttp.ClientSession() as session:
        async with session.post(f"{server_url}/push", json=payload, timeout=10) as response:
            data = await response.json(content_type=None)
            if response.status != 200 or data.get("code") not in (200, "200", None):
                raise ValueError(data.get("message") or f"Bark 通知发送失败: HTTP {response.status}")


async def _send_test_email_notification(config_data: Dict[str, Any], message: str) -> None:
    import smtplib
    ...
    email_password = (config_data.get("email_password") or "").strip()
    recipient_email = (config_data.get("recipient_email") or "").strip()
    smtp_use_ssl = bool(config_data.get("smtp_use_ssl", smtp_port == 465))
    smtp_use_tls = bool(config_data.get("smtp_use_tls", not smtp_use_ssl and smtp_port == 587))

    if not all([smtp_server, email_user, email_password, recipient_email]):
        raise ValueError("邮件通知配置不完整，请检查 SMTP 服务器、邮箱账号、密码和收件人")

    email_from = (config_data.get("email_from") or email_user).strip()
    subject = (config_data.get("subject") or "闲鱼自动回复测试通知").strip()
    ...
    try:
        ...
        raise ValueError(f"SMTP 认证失败: {exc}") from exc
    except smtplib.SMTPException as exc:
        raise ValueError(f"SMTP 发送失败: {exc}") from exc
    finally:
        if server:
            try:
                server.quit()
...
async def _send_test_webhook_notification(config_data: Dict[str, Any], message: str) -> None:
    import aiohttp

    webhook_url = (config_data.get("webhook_url") or "").strip()
    http_method = (config_data.get("http_method") or "POST").upper()
    if not webhook_url:
        raise ValueError("Webhook 地址不能为空")

    headers_raw = config_data.get("headers") or {}
    if isinstance(headers_raw, str):
        try:
            headers_raw = json.loads(headers_raw) if headers_raw else {}
        except json.JSONDecodeError:
            headers_raw = {}
        ...
        if http_method == "PUT":
            async with session.put(webhook_url, json=payload, headers=headers, timeout=10) as response:
                if response.status >= 400:
                    raise ValueError(f"Webhook 通知发送失败: HTTP {response.status}")
        else:
...
async def _send_test_wechat_notification(config_data: Dict[str, Any], message: str) -> None:
    import aiohttp

    webhook_url = (config_data.get("webhook_url") or "").strip()
    if not webhook_url:
        raise ValueError("企业微信 Webhook 地址不能为空")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(webhook_url, json=payload, timeout=10) as response:
            data = await response.json(content_type=None)
            if response.status != 200 or data.get("errcode") not in (0, "0", None):
                raise ValueError(data.get("errmsg") or f"企业微信通知发送失败: HTTP {response.status}")


async def _send_test_telegram_notification(config_data: Dict[str, Any], message: str) -> None:
    import aiohttp

    bot_token = (config_data.get("bot_token") or "").strip()
    chat_id = str(config_data.get("chat_id") or "").strip()
    if not bot_token or not chat_id:
        raise ValueError("Telegram 配置不完整，请检查 bot_token 和 chat_id")
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(api_url, json=payload, timeout=10) as response:
            ...
            if response.status != 200 or data.get("ok") is not True:
                raise ValueError((data or {}).get("description") or f"Telegram 通知发送失败: HTTP {response.status}")


async def _send_test_notification(channel_type: str, config_data: Dict[str, Any], channel_name: str) -> str:
    message = (
        "闲鱼自动回复管理系统测试通知\n\n"
        f"渠道名称: {channel_name}\n"
        f"渠道类型: {channel_type}\n"
    ...
    if normalized_type in ("ding_talk", "dingtalk"):
        await _send_test_dingtalk_notification(config_data, message)
    elif normalized_type in ("feishu", "lark"):
        await _send_test_feishu_notification(config_data, message)
    elif normalized_type == "bark":
        await _send_test_bark_notification(config_data, message)
    elif normalized_type == "email":
        await _send_test_email_notification(config_data, message)
    elif normalized_type == "webhook":
        await _send_test_webhook_notification(config_data, message)
    elif normalized_type == "wechat":
        await _send_test_wechat_notification(config_data, message)
    elif normalized_type == "telegram":
        await _send_test_telegram_notification(config_data, message)
    elif normalized_type == "qq":
        raise ValueError("QQ 通知测试暂未支持，请改用其他通知渠道或补充后端适配")
    else:
        raise ValueError(f"暂不支持的通知渠道类型: {channel_type}")

    return f"{channel_name} 测试消息发送成功"
...
async def test_notification_channel(channel_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    """测试发送通知渠道"""
    from db_manager import db_manager

    try:
        user_channels = db_manager.get_notification_channels(current_user['user_id'])
        channel = next((item for item in user_channels if item.get('id') == channel_id), None)
        if not channel:
            raise HTTPException(status_code=404, detail='通知渠道不存在')

        config_data = _parse_notification_channel_config(channel.get('config'))
        message = await _send_test_notification(
            channel.get('type', ''),
            config_data,
            channel.get('name', f'渠道 {channel_id}'),
        )
...
def get_all_message_notifications(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        user_cookies = db_manager.get_all_cookies(user_id)
        return {cid: notifications for cid, notifications in all_notifications.items() if cid in user_cookies}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_account_notifications(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    from db_manager import db_manager
    try:
        _ensure_cookie_access(cid, current_user, "访问")
...
def set_message_notification(cid: str, notification_data: MessageNotificationIn, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    from db_manager import db_manager
    try:
        _ensure_cookie_access(cid, current_user, "操作")
        # 检查通知渠道是否存在
...
def delete_message_notification(notification_id: int, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    from db_manager import db_manager
    try:
        notification = db_manager.get_message_notification(notification_id)
        if not notification:
            raise HTTPException(status_code=404, detail='通知配置不存在')
        _ensure_cookie_access(notification['cookie_id'], current_user, "操作")
...
def get_public_system_settings():
    ...
    try:
        ...
        # 允许通过环境变量覆盖
        if os.getenv('LOGIN_CAPTCHA_ENABLED'):
            result['login_captcha_enabled'] = os.getenv('LOGIN_CAPTCHA_ENABLED', 'true').lower()
...
def remove_cookie(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.remove_cookie(cid, current_user)
...
class AutoReplyOncePerCustomerUpdate(BaseModel):
    enabled: bool
...
def update_auto_confirm(cid: str, update_data: AutoConfirmUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.update_auto_confirm(cid, update_data, current_user)


@app.get("/cookies/{cid}/auto-confirm")
def get_auto_confirm(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_auto_confirm(cid, current_user)
...
def update_cookie_remark(cid: str, update_data: RemarkUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.update_cookie_remark(cid, update_data, current_user)


@app.get("/cookies/{cid}/remark")
def get_cookie_remark(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_cookie_remark(cid, current_user)
...
def update_cookie_pause_duration(cid: str, update_data: PauseDurationUpdate, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.update_cookie_pause_duration(cid, update_data, current_user)
...
def get_cookie_pause_duration(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_cookie_pause_duration(cid, current_user)


@app.put("/cookies/{cid}/auto-reply-once-per-customer")
def update_cookie_auto_reply_once_per_customer(
    cid: str,
    update_data: AutoReplyOncePerCustomerUpdate,
    current_user: Dict[str, Any] = Depends(get_current_user),
):
    return account_routes.update_cookie_auto_reply_once_per_customer(cid, update_data, current_user)


@app.get("/cookies/{cid}/auto-reply-once-per-customer")
def get_cookie_auto_reply_once_per_customer(cid: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return account_routes.get_cookie_auto_reply_once_per_customer(cid, current_user)
...
def create_card(card_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        log_with_user('warning', f"创建卡券参数冲突: {card_data.get('name', '未知')} - {str(e)}", current_user)
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
def update_card(card_id: int, card_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        success = db_manager.update_card(
            ...
            image_url=card_data.get('image_url'),
            description=card_data.get('description'),
            enabled=card_data['enabled'] if 'enabled' in card_data else None,
...
@app.get("/delivery-rules/{rule_id}/records")
def get_delivery_rule_records(rule_id: int, limit: int = 50, current_user: Dict[str, Any] = Depends(get_current_user)):
    """获取指定发货规则的自动发货明细记录"""
    try:
        from db_manager import db_manager
        user_id = current_user['user_id']
        records = db_manager.get_delivery_records(user_id=user_id, rule_id=rule_id, limit=max(1, min(limit, 200)))
...
def create_delivery_rule(rule_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        item_title = (rule_data.get('item_title') or '').strip()
        keyword = item_title or item_id or (rule_data.get('keyword') or '').strip()
        if not item_id:
            raise HTTPException(status_code=400, detail="请选择绑定商品")
        if not keyword:
...
def update_delivery_rule(rule_id: int, rule_data: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        item_title = rule_data.get('item_title')
        ...
        if keyword is None and item_id is not None:
        ...
        success = db_manager.update_delivery_rule(
            ...
            enabled=rule_data['enabled'] if 'enabled' in rule_data else None,
            description=rule_data.get('description'),
            user_id=user_id,
            item_id=item_id
        )
...
def export_backup(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        # 返回JSON响应，设置下载头
        response = JSONResponse(content=jsonable_encoder(backup_data))
...
def import_backup(file: UploadFile = File(...), current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="备份文件编码无效，请使用 UTF-8 JSON 文件")
    except Exception as e:
...
class ItemUpdatePayload(BaseModel):
    ...
    auto_relist_enabled: Optional[bool] = None
    auto_polish_enabled: Optional[bool] = None
    auto_polish_interval_hours: Optional[int] = None


async def execute_item_operation(cookie_id: str, item_id: str, operation: str) -> Dict[str, Any]:
    """执行商品操作。

    优先通过浏览器自动化执行真实商品操作，成功后再同步更新数据库状态。
    """
    cookie_details = db_manager.get_cookie_details(cookie_id)
    cookie_info = cookie_details or db_manager.get_cookie_by_id(cookie_id)
    if not cookie_info:
        return {"success": False, "message": "账号不存在"}
    ...
    xianyu_instance = XianyuLive(cookies_str, cookie_id, user_id=cookie_details.get('user_id') if cookie_details else None)
    try:
        await xianyu_instance.create_session()
        execution_result = await xianyu_instance.execute_item_management_action(
            item_id=item_id,
            operation=operation,
        ...
        if operation == "polish":
            db_manager.mark_item_polished(cookie_id, item_id)
            return {
                "success": True,
                "message": execution_result.get("message") or "商品擦亮成功",
                "operation": operation,
                "executed_by": execution_result.get("executed_by", "browser"),
            }

        if operation == "relist":
            db_manager.mark_item_relisted(cookie_id, item_id)
            return {
                "success": True,
                "message": execution_result.get("message") or "商品上架成功",
                "operation": operation,
                "executed_by": execution_result.get("executed_by", "browser"),
            }

        return {"success": False, "message": f"不支持的商品操作: {operation}"}
    except Exception as e:
        logger.error(f"执行商品操作失败: cookie_id={cookie_id}, item_id={item_id}, operation={operation}, error={e}")
        return {"success": False, "message": f"执行失败: {str(e)}", "operation": operation}
    finally:
        try:
...
async def item_automation_loop():
    """商品自动任务巡检循环"""
    while True:
        try:
            ...
            for item in all_items:
                ...
                auto_relist_enabled = bool(item.get('auto_relist_enabled'))
                auto_polish_enabled = bool(item.get('auto_polish_enabled'))
                interval_hours = item.get('auto_polish_interval_hours') or 24

                if auto_relist_enabled and item_status in {"sold", "sold_out", "end", "removed", "off_shelf", "2", "3"}:
                    result = await execute_item_operation(cookie_id, item_id, "relist")
                    if result.get("success"):
                        logger.info(f"商品自动上架成功: {cookie_id}/{item_id}")

                if auto_polish_enabled:
                    last_polish_at = item.get('last_polish_at')
                    should_polish = True
                    if last_polish_at:
                        try:
                            last_ts = time.mktime(time.strptime(str(last_polish_at), '%Y-%m-%d %H:%M:%S'))
                            should_polish = (current_ts - last_ts) >= int(interval_hours) * 3600
                        except Exception:
                            should_polish = True

                    if should_polish:
                        result = await execute_item_operation(cookie_id, item_id, "polish")
                        if result.get("success"):
                            logger.info(f"商品自动擦亮成功: {cookie_id}/{item_id}")
        except Exception as e:
            logger.error(f"商品自动任务巡检异常: {e}")
...
def update_item_detail(
    ...
    try:
        ...
        image_urls = update_payload.pop('image_urls', None)
        primary_image_url = update_payload.pop('primary_image_url', None)

        if primary_image_url is not None and image_urls is None:
            current_image_urls = [
                image.get('image_url')
                for image in (existing_item.get('images') or [])
                if image.get('image_url')
            ]
            normalized_primary = primary_image_url.strip()
            if normalized_primary:
                image_urls = [normalized_primary] + [url for url in current_image_urls if url != normalized_primary]
            else:
                image_urls = current_image_urls
        if 'auto_polish_interval_hours' in update_payload:
            interval_hours = int(update_payload['auto_polish_interval_hours'])
            if interval_hours < 1 or interval_hours > 720:
                raise HTTPException(status_code=400, detail="自动擦亮间隔必须在 1-720 小时之间")
            update_payload['auto_polish_interval_hours'] = interval_hours

        if image_urls is not None:
            normalized_image_urls = []
            for image_url in image_urls:
                clean_url = str(image_url or '').strip()
                if clean_url and clean_url not in normalized_image_urls:
                    normalized_image_urls.append(clean_url)
            if not db_manager.replace_item_images(cookie_id, item_id, normalized_image_urls):
                raise HTTPException(status_code=400, detail="商品图片更新失败")

        field_update_success = True
        if update_payload:
            field_update_success = db_manager.update_item_fields(cookie_id, item_id, update_payload)
        ...
        raise HTTPException(status_code=500, detail=f"更新商品信息失败: {str(e)}")


@app.post("/items/{cookie_id}/{item_id}/polish")
async def polish_item(
    ...
    if not result.get("success"):
        raise HTTPException(status_code=400, detail=result.get("message", "擦亮失败"))
    return result
...
async def relist_item(
    ...
    result = await execute_item_operation(cookie_id, item_id, "relist")
    if not result.get("success"):
        raise HTTPException(status_code=400, detail=result.get("message", "上架失败"))
    return result
...
async def batch_delete_item_reply(
    ...
    for item in req.items:
        _ensure_cookie_access(item.cookie_id, current_user, "访问")
...
async def upload_database_backup(admin_user: Dict[str, Any] = Depends(require_admin),
    ...
    try:
        ...
        db_dir = os.path.dirname(os.path.abspath(current_db_path)) or os.getcwd()
        temp_file_path = os.path.join(db_dir, f"temp_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db")
...
def list_backup_files(admin_user: Dict[str, Any] = Depends(require_admin)):
    ...
    try:
        ...
        db_dir = os.path.dirname(os.path.abspath(db_manager.db_path)) or os.getcwd()
        backup_pattern = os.path.join(db_dir, "xianyu_data_backup_*.db")
...
def get_user_orders(
    ...
    return order_routes.get_user_orders(cookie_id, status, keyword, page, page_size, current_user)


@app.post('/api/orders/fetch')
async def fetch_user_orders(request: dict, current_user: Dict[str, Any] = Depends(get_current_user)):
    return await order_routes.fetch_user_orders(request, current_user)


@app.get('/api/orders/{order_id}')
def get_order_detail(order_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return order_routes.get_order_detail(order_id, current_user)


@app.delete('/api/orders/{order_id}')
def delete_order(order_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    return order_routes.delete_order(order_id, current_user)
...
API_PREFIXES = ['/api/', '/static/', '/netdisk_api/', '/health', '/login', '/logout', '/register', '/verify', '/change-password', '/change-admin-password']
