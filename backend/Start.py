...
def _start_api_server():
    ...
    port = int(api_conf.get('port', 8080))

    if 'url' in api_conf and 'host' not in api_conf and 'port' not in api_conf:
        url = api_conf.get('url', 'http://127.0.0.1:8080/xianyu/reply')
        parsed = urlparse(url)
        if parsed.hostname and parsed.hostname != 'localhost':
            host = parsed.hostname
        port = parsed.port or 8080

    env_host = (os.getenv('API_HOST') or '').strip()
    if env_host:
        host = env_host

    env_port = (os.getenv('API_PORT') or '').strip()
    if env_port:
        port = int(env_port)
...
async def main():
    ...
    for cid, val in manager.cookies.items():
        ...
        try:
            ...
            logger.info(f"启动 Cookie 任务: {cid} (用户ID: {user_id}, 当前任务数: {len(manager.tasks)})")
        except Exception as e:
...
if __name__ == '__main__':
    # Python 3.14+ 中 get_event_loop() 不会在主线程自动创建循环，
    # 直接使用 asyncio.run() 并处理 Ctrl+C，避免启动阶段 RuntimeError。
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        logger.info("收到中断信号，正在退出程序...")
